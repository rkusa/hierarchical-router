...
Route.prototype.execute = function(context, req, args, cont) {
  ...
  return function*() {
    ...
    args = self.method === 'init' ? [context.ctx].concat(args) : args

    if (self.action.constructor.name === 'GeneratorFunction') {
      yield *self.action.apply(req, args.concat(cont))
    } else {
      yield function(callback) {
        var done = co(function*() {
          yield cont
          callback()
        })
        done.redirect = function(url, opts) {
          co(cont.redirect(url, opts))()
        }

        self.action.apply(req, args.concat(done))
      }
    }
  }
}
...
Route.prototype.executeUntil = function(to, context, trigger) {
  ...
  return function*() {
    ...
    if (from === to) {
      ...
      route = to
      var tmp = !!!~stack.indexOf(to) ? [to] : []
      while((route = route.parent)) {
        // count the relevant sections of the pattern
        var relevant   = (route.pattern.match(/\//g) || []).length

        // count the relevant sections of the full path to calculate the
        ...
        // split into sections
        var lhs = context.location.path.split('/').splice(irrelevant + 1, relevant)
          , rhs = context.path.split('/').splice(irrelevant + 1, relevant)
        for (var i = 0; i < lhs.length; ++i) {
          if (lhs[i] !== rhs[i]) {
            // parameter has changed
            stack.push.apply(stack, tmp)
            revert.push.apply(revert, tmp)
            tmp = []
            break
          }
        }
    ...
    var finalize = function*() {
      ...
      router.emit('navigated', context.path, context.method, context)
