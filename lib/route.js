...
Route.prototype.execute = function(context, args, cont) {
  if (this.rdy) res.ready(this.rdy, !initialized)
  ...
  return function*() {
    ...
    if (self.method === 'init') {
      yield *self.action.apply(context, [req.ctx].concat(args, cont))
    } else {
      context.$state._currentRoute = self.method.toUpperCase() + ' ' + self.path
      yield *self.action.apply(context, args.concat(cont))
    }
  }
}
...
Route.prototype.revert = function(context) {
  if (isServer) return
  debug('Revert %s ', this.path)
  this.router.emit('revert', this, context)
}
...
Route.prototype.createChild = function(method, pattern, action) {
  ...
  var handler = function(context, trigger) {
    return function*() {
      // TODO: init location
      var from = context.$state._location ? context.$state._location : new Route(router, '', '', function(_, cb) { cb() })
...
Route.prototype.executeUntil = function(to, context, trigger) {
  ...
  return function*() {
    ...
    var finalize = function*() {
      initialized = true
      console.log('finalize', context.$state)
      context.$state._location = route
      // context.$state._path = context.path
    ...
    var cont = function *cont() {
      ...
      else yield route.execute(context, args, next)
    }
    cont.end      = finalize
